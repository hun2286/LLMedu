...
# 저장할 텍스트 경로
txt_path = r"C:\Users\BGR_NC_2_NOTE\Desktop\pdfs\example_ocr.txt"

# Tesseract 경로
pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
os.environ["TESSDATA_PREFIX"] = r"C:\Program Files\Tesseract-OCR\tessdata"

def ocr_pdf(pdf_path, max_pages=2):  # max_pages 매개변수 추가
    text = ""
    with fitz.open(pdf_path) as pdf:
        for page_num, page in enumerate(pdf, start=1):
            ...
            img = img.convert("L")
            page_text = pytesseract.image_to_string(img, lang="kor+chi_tra", config="--oem 3 --psm 6")
            text += f"--- 페이지 {page_num} ---\n"
            text += page_text + "\n\n"
    return text
